        ...
        <div class="rounded-md border p-4">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div class="grid gap-1.5">
                    ...
                    <Select :model-value="form.customer_id" @update:model-value="(v) => (form.customer_id = v)">
                ...
                <div class="grid gap-1.5">
                    <Label>担当者</Label>
                    <Select :model-value="form.employee_id || '__none__'"
                ...
                <!-- 見積日 -->
                <div class="grid gap-1.5">
                    <Label>見積日 <span class="text-destructive">*</span></Label>
                    <Input type="date" v-model="form.quote_date"
                ...
                <div class="grid gap-1.5">
                    ...
                    <Input type="date" v-model="form.expiry_date"
                        :class="{ 'border-destructive': form.errors.expiry_date }" />
                ...
                <div class="grid gap-1.5 sm:col-span-2">
                    ...
                    <Input v-model="form.subject" placeholder="例: ○○システム導入費用"
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="bg-muted/50">
                        <tr>
                            <th class="px-3 py-2 text-left font-medium">商品選択</th>
                            <th class="px-3 py-2 text-left font-medium">品名 <span class="text-destructive">*</span></th>
                            ...
                            <th class="px-3 py-2 text-right font-medium">単価 <span class="text-destructive">*</span></th>
                    ...
                    <tbody>
                        <tr v-for="(item, i) in form.items" :key="i" class="border-t">
                            ...
                            <td class="px-2 py-1.5">
                                <Select :model-value="item.product_id ? String(item.product_id) : '__none__'"
                                    @update:model-value="(v) => onProductChange(i, v)">
                            ...
                            <!-- 品名 -->
                            <td class="px-2 py-1.5">
                                <Input v-model="item.product_name" class="h-8 min-w-36"
                                    :class="{ 'border-destructive': getItemError(i, 'product_name') }" />
                            </td>
                            <!-- 仕様 -->
                            <td class="px-2 py-1.5">
                            ...
                            <!-- 単価 -->
                            <td class="px-2 py-1.5">
                                <Input v-model="item.unit_price" type="number" step="1" min="0"
                                    class="h-8 w-24 text-right tabular-nums"
                                    :class="{ 'border-destructive': getItemError(i, 'unit_price') }"
                                    @input="recalcItem(i)" />
                            </td>
                            <!-- 税率 -->
                            <td class="px-2 py-1.5">
                                <Select :model-value="item.tax_rate"
                                    @update:model-value="(v) => { item.tax_rate = v; recalcItem(i); }">
                                    <SelectTrigger class="h-8 w-20">
                                        <SelectValue />
                                    </SelectTrigger>
                                    <SelectContent>
                                        <SelectItem v-for="(label, key) in taxRates" :key="key" :value="key">{{ label }}</SelectItem>
                                    </SelectContent>
                                </Select>
                            </td>
                            <!-- 金額 -->
                            ...
                            <td class="px-2 py-1.5 text-center">
                                <Button type="button" variant="ghost" size="icon" class="h-8 w-8 text-destructive hover:text-destructive"
                    ...
                    <tfoot class="border-t bg-muted/30 font-medium">
                        ...
                        <tr class="text-base">
                            ...
                            <td class="px-3 py-2 text-right tabular-nums font-bold">{{ fmt(totalAmount) }}</td>
                            <td colspan="2"></td>
                        </tr>
                    </tfoot>
                </table>
