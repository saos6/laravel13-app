...
import { ArrowUpDown, ArrowUp, ArrowDown, Download, Plus, Pencil, Trash2, Columns3, Search, Eye } from 'lucide-vue-next';
...
interface Props {
    quotes: {
        data: Quote[];
        total: number; per_page: number; current_page: number;
...
type ColumnKey = 'quote_number' | 'customer' | 'employee' | 'quote_date' | 'expiry_date' | 'subject' | 'status' | 'total_amount' | 'created_at' | 'updated_at';
...
function applyFilters() {
    router.get(QuoteController.index.url(),
        { search: search.value, status: status.value, sort: sortField.value, direction: sortDir.value, per_page: perPage.value },
...
function exportExcel() {
    const params = new URLSearchParams({ search: search.value, status: status.value, sort: sortField.value, direction: sortDir.value });
        ...
        <div class="flex flex-col gap-4 p-4">
                ...
                <div class="flex gap-2">
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" class="w-44">
                            ...
                            <DropdownMenuCheckboxItem v-for="(col, key) in columns" :key="key" :checked="col.visible"
                                @update:checked="(v) => (columns[key as ColumnKey].visible = v)">
                                {{ col.label }}
                            </DropdownMenuCheckboxItem>
                        </DropdownMenuContent>
                    </DropdownMenu>
                    <Button variant="outline" size="sm" @click="exportExcel"><Download class="mr-1 h-4 w-4" />Excel出力</Button>
                    <Button size="sm" as-child>
                        <Link :href="QuoteController.create.url()"><Plus class="mr-1 h-4 w-4" />新規作成</Link>
                    </Button>
                </div>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-2">
                <div class="relative max-w-sm flex-1">
                    ...
                    <Input v-model="search" placeholder="見積番号・件名・得意先で検索..." class="pl-8" @keyup.enter="handleSearch" />
                </div>
                <Select :model-value="status || '__all__'"
                ...
                <Button variant="ghost" size="sm" @click="() => { search = ''; status = ''; handleSearch(); }">クリア</Button>
            </div>

            <!-- 件数 -->
            <div class="flex items-center justify-between text-sm text-muted-foreground">
                <span>
                    全 <strong class="text-foreground">{{ quotes.total }}</strong> 件
                    <template v-if="quotes.from && quotes.to">（{{ quotes.from }}〜{{ quotes.to }} 件表示）</template>
                </span>
                ...
                <table class="w-full text-sm">
                    <thead class="bg-muted/50">
                        <tr>
                            <template v-for="(col, key) in columns" :key="key">
                                <th v-if="col.visible"
                                    class="select-none px-4 py-3 text-left font-medium whitespace-nowrap"
                    ...
                    <tbody>
                        <tr v-for="quote in quotes.data" :key="quote.id"
                            ...
                            <td v-if="columns.quote_number.visible" class="px-4 py-3 font-mono text-sm">{{ quote.quote_number }}</td>
                            <td v-if="columns.customer.visible" class="px-4 py-3">{{ quote.customer?.name ?? '—' }}</td>
                            <td v-if="columns.employee.visible" class="px-4 py-3 text-muted-foreground">{{ quote.employee?.name ?? '—' }}</td>
                            <td v-if="columns.quote_date.visible" class="px-4 py-3 text-muted-foreground whitespace-nowrap">
                                {{ quote.quote_date ? new Date(quote.quote_date).toLocaleDateString('ja-JP') : '—' }}
                            </td>
                            <td v-if="columns.expiry_date.visible" class="px-4 py-3 text-muted-foreground whitespace-nowrap">
                                {{ quote.expiry_date ? new Date(quote.expiry_date).toLocaleDateString('ja-JP') : '—' }}
                            </td>
                            <td v-if="columns.subject.visible" class="max-w-xs truncate px-4 py-3">{{ quote.subject }}</td>
                            ...
                            <td v-if="columns.created_at.visible" class="px-4 py-3 text-muted-foreground whitespace-nowrap">
                                {{ quote.created_at ? new Date(quote.created_at).toLocaleString('ja-JP') : '—' }}
                            </td>
                            <td v-if="columns.updated_at.visible" class="px-4 py-3 text-muted-foreground whitespace-nowrap">
                                {{ quote.updated_at ? new Date(quote.updated_at).toLocaleString('ja-JP') : '—' }}
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex gap-1">
                                    <Button variant="ghost" size="icon" class="h-8 w-8" as-child>
                                        <Link :href="QuoteController.show.url(quote.id)"><Eye class="h-4 w-4" /></Link>
                                    </Button>
                                    <Button variant="ghost" size="icon" class="h-8 w-8" as-child>
                                        <Link :href="QuoteController.edit.url(quote.id)"><Pencil class="h-4 w-4" /></Link>
                                    </Button>
                                    <Button variant="ghost" size="icon" class="h-8 w-8 text-destructive hover:text-destructive"
                                        @click="deleteQuote(quote.id, quote.quote_number)">
            ...
            <!-- ページング -->
            <div v-if="quotes.last_page > 1" class="flex items-center justify-center gap-1">
